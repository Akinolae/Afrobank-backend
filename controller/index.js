...
class Customer {
  ...
  register = async (
    ...
    if (joi_validate.error) {
      ...
      try {
        ...
        this.sendMail(
          user_reg.emailTemplate(firstName, pin, accountBalance, accountNumber),
          email,
  ...
  getBalance = async (accountNumber, res) => {
    ...
    !data.status
      ? response(data.message, false, statusCode.StatusCodes.BAD_REQUEST, res)
            ...
            if (isOtpValid) {
              ...
              this.customer
                .updateOne(
                  ...
                  { $set: { accountBalance: newSenderBalance, otp: null } }
                )
                .then(() => {
                  response(
              ...
              this.customer
                .updateOne(
                  ...
                  { $set: { accountBalance: newRecipientBalance } }
                )
                .then(() => {
                  this.sendMail(
  ...
  sendOtp = async (payload) => {
    ...
    try {
      ...
      await this.customer
        .updateOne({ accountNumber: accountNumber }, { $set: { otp: otp } })
        .then(() => {
          this.sendMail(
